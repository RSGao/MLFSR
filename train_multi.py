...
def main(args):
    ...
    net, optimizer, criterion, train_loader = acc.prepare(net, optimizer, criterion, train_loader)
    
    # set tblogger
    writer = SummaryWriter(log_dir=os.path.join(args.tbpath, args.model_name))  # log_dir是日志存放的目录，需要自己设置
    global_iter = 0
    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        ''' Training '''
        loss_epoch_train, psnr_epoch_train, ssim_epoch_train, global_iter = train(args, train_loader, net, criterion, optimizer, acc, writer, global_iter)
...
def train(args, train_loader, net, criterion, optimizer, acc, writer, global_iter):
