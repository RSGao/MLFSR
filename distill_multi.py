...
from model.SR.EPIT_distill import EPIT 
...
def main(args):
    ...
    # Teacher net
    teachernet = EPIT()
    teachernet.distill = True
    teacher_ckpt = torch.load('/data1/gaors/BasicLFSR/pth/EPIT_5x5_4x_model.pth')['state_dict'] 
    teachernet.load_state_dict(teacher_ckpt)
    teachernet.eval()
    ...
    net, teachernet, optimizer, criterion, dist_loss, train_loader = acc.prepare(net, teachernet, optimizer, criterion, dist_loss, train_loader)
    
    # set tblogger
    writer = SummaryWriter(log_dir=os.path.join(args.tbpath, args.model_name))
    global_iter = 0
    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        ''' Training '''
        loss_epoch_train, psnr_epoch_train, ssim_epoch_train, global_iter = train(args, train_loader, teachernet, net, criterion, dist_loss, optimizer, acc, writer, global_iter)
        ...
        if (idx_epoch + 1)%step==0 or idx_epoch > args.epoch-step:
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    psnr_iter_test, ssim_iter_test, LF_name = test_whole(test_loader, net, acc, save_dir)  ##
...
def train(args, train_loader, teachernet, net, criterion, dist_loss, optimizer, acc, writer, global_iter):
    ...
    loss_iter_train = []
    ssim_iter_train = []
    for idx_iter, (data, label, data_info) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70, disable=not acc.is_main_process):
        ...
        # Get EPIT output
        with torch.no_grad():
            _, teacherdeep = teachernet(data, data_info)
            
        
        out, studeep = net(data, data_info)
        
        # Hybrid loss (recon + distillation)
        recon_loss = criterion(out, label, data_info)
        distillation_loss1 = dist_loss(teacherdeep, studeep) * 0.1
        loss = recon_loss + distillation_loss1
    ...
    return loss_epoch_train, psnr_epoch_train, ssim_epoch_train, global_iter
...
def test(test_loader, device, net, save_dir=None):
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        psnr, ssim, PSNR, SSIM = cal_metrics(args, Hr_SAI_y, Sr_SAI_y) ##
    ...
    return psnr_iter_test, ssim_iter_test, LF_iter_test, view_psnr

# full-image inference
def test_whole(test_loader, device, net, save_dir=None):
    LF_iter_test = []
    psnr_iter_test = []
    ssim_iter_test = []
    view_psnr = []
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        with torch.no_grad():
            ...
            Sr_SAI_y = net(Lr_SAI_y.unsqueeze(0).unsqueeze(0), data_info)
            if isinstance(Sr_SAI_y, tuple):
                Sr_SAI_y = Sr_SAI_y[0]
...
if __name__ == '__main__':
    from distill_option import args
    main(args)
